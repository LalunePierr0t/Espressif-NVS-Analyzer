        ...
        if self.size == 0:
            ...
            if self.chunks[0][0] != self.chunk_start:
                raise ValidationError(
                    "BLOB parsing error: Starting chunk index %d does not match chunk start value %d from BLOB index."
                    % (self.chunks[0][0], self.chunk_start)
                )
                return bytearray()
            if len(self.chunks) != self.chunk_count:
                raise ValidationError(
                    "BLOB parsing error: Number of chunks being reassembled %d does not match chunk count %d from BLOB index."
...
def verify_nvs_size(input):
    if (len(input) % PAGE_SIZE) != 0:
        message = (
            "Partition size (%d bytes) is not a multiple of page size (%d bytes)"
            % (len(input), PAGE_SIZE)
        )
...
def build_page_array(input):
    ...
    for page_index in range(num_pages):
        ...
        page_state = int.from_bytes(
            input[
                (page_base_address + PAGE_STATE_OFFSET) : (
                    page_base_address + PAGE_STATE_OFFSET + 4
                )
            ],
            byteorder="little",
            signed=False,
        ...
        seq_no = int.from_bytes(
            input[
                (page_base_address + PAGES_SEQ_NO_OFFSET) : (
                    page_base_address + PAGES_SEQ_NO_OFFSET + 4
                )
            ],
            byteorder="little",
            signed=False,
        ...
        if entry_states[i] == EntryState.WRITTEN:
            ...
            if entry_type < 0x20:
                ...
                entry_data = int.from_bytes(
                    page_data[
                        entry_base + DATA_OFFSET : entry_base + DATA_OFFSET + num_bytes
                    ],
                    byteorder="little",
                    signed=is_signed,
                )
                ...
                data_size = int.from_bytes(
                    page_data[entry_base + DATA_OFFSET : entry_base + DATA_OFFSET + 2],
                    byteorder="little",
                    signed=False,
                )
                ...
                blob_size = int.from_bytes(
                    page_data[entry_base + DATA_OFFSET : entry_base + DATA_OFFSET + 4],
                    byteorder="little",
                    signed=False,
                )
                ...
                do_not_add = True
                print(
                    "  Unknown entry type 0x%02X for entry %d at offset 0x%X in page %d"
                    % (entry_type, i, entry_base, page_index)
                )
...
def dump_nvs_data():
    for ns_name in nvs_table:
        ...
        for entry_key in ns_dict:
            ...
            print(
                "  %-16s: %-4s  %s" % (entry_key, EntryType(entry_type).name, data_str)
            )


def dump_nvs_data_csv(p_output_file_path: str):
    """
    Dump the NVS data into a CSV file.

    :param p_output_file_path : str : The path to the output CSV file.
    """
    ...
    with open(p_output_file_path, "w") as output_file:
        ...
        for ns_name in nvs_table:
            namespace = str(ns_name) + ",namespace,,"
            output_file.write(namespace + os.linesep)
            ns_dict = nvs_table[ns_name]
            for entry_key in ns_dict:
                entry_info = ns_dict[entry_key]
                ...
                # test if entry_data is a byte array
                if isinstance(entry_data, bytes):
                    entry_data = entry_data.hex()
                # test if entry_data is a integer
                elif isinstance(entry_data, int):
                    entry_data = str(entry_data)
                else:
                    entry_data = entry_data.hex()
...
def main():
    ...
    parser.add_argument(
        "input",
        help="Path to binary dump of NVS partition.",
        type=argparse.FileType("rb"),
    )
    parser.add_argument(
        "--blob_limit",
        "-b",
        help="Set maximum number of bytes to dump per BLOB (0 = no limit)",
        type=int,
        default=0,
    )
    parser.add_argument(
        "--special_handling",
        "-s",
        help="Display SSID and password BLOBs as strings",
        action="store_true",
    )
    parser.add_argument(
        "--csv",
...
class ValidationError(InputError):
    def __init__(self, message):
        super(ValidationError, self).__init__("NVS partition invalid: %s" % (message))
